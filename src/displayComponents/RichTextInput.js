...
class RichTextInput extends React.Component {
	...
	render() {
		const { tags, loadDraft, errorText, required, translate, styles, stylesQuill, placeholder } = this.props;
		const modules = {
			toolbar: {
				container: [
					...
					<GridItem xs={12}>
						...
						>
							<ReactQuill
								...
								id={this.props.id}
								style={{ ...stylesQuill }}
								className={`text-editor ${this.props.quillEditorButtonsEmpty} ${!!errorText ? 'text-editor-error' : ''}`}
							/>
						</div>
					</GridItem>
				</Grid>
