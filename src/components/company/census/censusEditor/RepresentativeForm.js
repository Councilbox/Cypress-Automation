...
import { Checkbox, Grid, GridItem, SelectInput, TextInput } from '../../../../displayComponents';
...
const RepresentativeForm = ({ updateState, translate, state, errors, languages }) => {
    ...
    return (<Grid>
        ...
        {state.hasRepresentative && <Fragment>
                ...
                >
                    {languages.map((language) => {
                        return <MenuItem value={language.columnName}
                                         key={`languagerepresentative_${language.columnName}`}>{language.desc}</MenuItem>
                    })}
                </SelectInput>
            </GridItem>
        </Fragment>}
