...
import { Grid, GridItem, MajorityInput, RichTextInput, SelectInput, TextInput } from '../../../displayComponents';
...
const CompanyDraftForm = ({ translate, draft, errors, updateState, companyStatutes, draftTypes, votingTypes, majorityTypes }) => (
    <Grid spacing={16}>
        ...
        <GridItem xs={12} lg={3} md={3}>
            ...
            >
                {companyStatutes.map((council) => {
                    return <MenuItem value={council.id}
                                     key={`counciltype_${council.id}`}>{translate[ council.title ] || council.title}</MenuItem>
                })}
            </SelectInput>
        </GridItem>
        <GridItem xs={12} lg={3} md={3}>
            ...
            >
                {draftTypes.map((draft) => {
                    return <MenuItem value={'' + draft.value}
                                     key={`draftType_${draft.value}`}>{translate[ draft.label ]}</MenuItem>
                })}
            </SelectInput>
        </GridItem>
        <GridItem xs={12} lg={3} md={3}>
            {draft.type === 1 && <SelectInput
            ...
            >
                {votingTypes.map((votingType) => {
                    return <MenuItem value={'' + votingType.value}
                                     key={`votingTypeType_${votingType.value}`}>{translate[ votingType.label ]}</MenuItem>
                })}
            </SelectInput>}
        </GridItem>
        ...
        {CBX.hasVotation(draft.votationType) && <Fragment>
            ...
            <GridItem xs={6} lg={3} md={3}>
                {CBX.majorityNeedsInput(draft.majorityType) && <MajorityInput
