...
class RescheduleModal extends Component {
    ...
    updateDate = (firstDate = this.state.dateStart, secondDate = this.state.dateStart2NdCall) => {
        ...
        if (!CBX.checkSecondDateAfterFirst(firstDate, secondDate)) {
            ...
            this.updateState({
                dateStart: firstDate,
                dateStart2NdCall: CBX.addMinimunDistance(firstDate, this.props.council.statute)
            });
        } else {
            if (!CBX.checkMinimunDistanceBetweenCalls(firstDate, secondDate, this.props.council.statute)) {
                this.updateState({
                    error2NdCall: translate.new_statutes_hours_warning.replace('{{hours}}', this.props.council.statute.minimumSeparationBetweenCall)
                });
            }
        }
    };
