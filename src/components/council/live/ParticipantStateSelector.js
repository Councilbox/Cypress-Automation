...
const StateIconButton = ({ loading, action, icon, active }) => (
    <div
        style={{border: `1px solid ${getSecondary()}`, width: '3.9em', backgroundColor: active? 'lightgrey' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center'}}
        onClick={() => action()}
    >
        {loading?
...
class ParticipantStateSelector extends Component {
    ...
    updateLiveParticipant = async (state, index, delegate, id = this.props.participant.id) => {
    ...
    render(){
        ...
        return(
            <div style={{width: '100%', display: 'flex', flexDirection: 'row'}}>
                ...
                {CBX.canHaveRepresentative(participant) && !participant.delegatedVotes.length > 0 &&
                    <div
                        style={{border: `1px solid ${getSecondary()}`, width: '3.9em', backgroundColor: participant.state === PARTICIPANT_STATES.REPRESENTATED? 'lightgrey' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center'}}                                                                 
                        onClick={() => this.setState({
                            addRepresentative: true
                        })}
                    >
                        <ParticipantStateIcon
                            participant={{state: PARTICIPANT_STATES.REPRESENTATED}}
                            tooltip={'change'}
                            translate={translate}
                        />
                    </div>
                ...
                {CBX.canDelegateVotes(council.statute, participant) &&
                    <React.Fragment>
                        ...
                        />
                        {!CBX.delegatedVotesLimitReached(council.statute, participant.delegatedVotes.length) &&
                            participant.state !== PARTICIPANT_STATES.DELEGATED &&
                                <BasicButton
                                    text={translate.add_delegated}
                                    color={'white'}
                                    onClick={() => this.setState({delegateVote: true})}
                                    buttonStyle={{marginRight: '1em', border: `2px solid ${getPrimary()}`}}
                                />
...
export default graphql(updateLiveParticipant, {name: 'updateLiveParticipant'})(ParticipantStateSelector);