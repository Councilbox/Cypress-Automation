...
import { BasicButton, LoadingSection, DropDownMenu, Grid, GridItem, AlertConfirm, TextInput } from "../displayComponents";
...
class CouncilEditorPreview extends Component {
    ...
    resetConveneTestValues = () => {
        this.setState({conveneTestModal: false, conveneTestSuccess: false, errors: {conveneTestEmail: ''} });
    ...
    _renderPreConveneModalBody = () => {
        if(this.state.preConveneSuccess){
            return(
                <SuccessMessage message={this.props.translate.sent} />
            );
        }
            ...
            <div style={{width: '100%', height: '100%', padding: '2em'}}>
                <Grid>
                    <GridItem xs={12} lg={12} md={12}>
                        ...
                        />  
                        <div style={{display: 'inline-block', width: '5em', marginTop: '5px'}}>               
                            <DropDownMenu
                                color="transparent"
                                buttonStyle={{
                                ...
                                text={<FontAwesome
                                    name={'bars'}
                                    style={{cursor: 'pointer', fontSize: '0.9em', height: '0.9em', color: secondary}}
                                /> }
                                ...
                                items={
                                    <Fragment>
                                        <MenuItem onClick={() => this.setState({conveneTestModal: true})}>{translate.send_test_convene}</MenuItem>
                                        <MenuItem onClick={() => this.setState({preConveneModal: true})}>{translate.send_preconvene}</MenuItem>
                                        <MenuItem onClick={() => this.setState({sendConveneWithoutNoticeModal: true})}>{translate.new_save_convene}</MenuItem>
                                    </Fragment>
                                }
                            />
                        </div>
                ...
                <AlertConfirm
                    ...
                    acceptAction={this.state.conveneTestSuccess?
                ...
                />
                <AlertConfirm
                    requestClose={() => this.setState({preConveneModal: false, preConveneSuccess: false})}
                    open={this.state.preConveneModal}
                    acceptAction={this.state.preConveneSuccess? () => this.setState({preConveneSuccess: false, preConveneModal: false}) : this.sendPreConvene}
