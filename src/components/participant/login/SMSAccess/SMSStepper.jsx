	...
	return (
		<Stepper nonLinear alternativeLabel style={{ height: '10em' }} >
			<Step className={`stepperAcceso${addClass}`}>
				<StepLabel >
					<span style={{ color: color || primary }}>{translate.room_access}</span>
				</StepLabel>
			</Step>
			...
			<Step className={`stepperAccesoNoActived${addClass}`}>
				<StepLabel>
					<span style={{ color: color || primary }}>{translate.validate_key}</span>
				</StepLabel>
			</Step>
		</Stepper>
	);
