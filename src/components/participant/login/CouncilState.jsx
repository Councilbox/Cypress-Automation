...
import { councilIsInTrash, councilIsNotLiveYet, councilIsNotCelebrated, councilIsFinished } from '../../../utils/CBX';
import { getPrimary, getSecondary, lightGrey, lightTurquoise } from '../../../styles/colors';
...
class TextRender extends React.PureComponent {
    ...
    render() {
        const { title, text, isHtmlText, hasCouncilInfo, council, company } = this.props;
...
class CouncilInfoCardRender extends React.PureComponent {
    ...
    render() {
        ...
        return (
            <React.Fragment>
                <div style={{backgroundColor: lightTurquoise, borderRadius: '4px'}}>
                    <CardHeader
                        avatar={
                            <Avatar src={company.logo} aria-label="CouncilLogo"/>
                        }
                        title={council.name}
                        subheader={moment(new Date(council.dateStart)).format('LLL')}
                        style={{paddingBottom: '0px'}}
                    />
                    <CardContent style={{paddingBottom: '16px'}}>
                        <p dangerouslySetInnerHTML={{ __html:  council.conveneText}}></p>
                    </CardContent>
                </div>
            </React.Fragment>
        );
