...
const SignUpStepper = ({ active, translate, windowSize, goToPage }) => {
    ...
    if (windowSize !== 'xs') {
        return (<Stepper activeStep={active} orientation={"vertical"}>
                <Step style={({ cursor: (active + 1) > 1 ? 'pointer' : 'inherited' })}
                      onClick={() => goToPage(1)}>
                    <StepLabel>{translate.company_data}</StepLabel>
                    <StepContent></StepContent>
                </Step>
                <Step style={({ cursor: (active + 1) > 2 ? 'pointer' : 'inherited' })}
                      onClick={() => goToPage(2)}>
    ...
    return (<Stepper activeStep={active} orientation={"horizontal"}>
            <Step style={({ cursor: (active + 1) > 1 ? 'pointer' : 'inherited' })}
                  onClick={() => goToPage(1)}>
                <StepLabel>
