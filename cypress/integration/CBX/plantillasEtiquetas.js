...
export default () => {
    ...
    cy.get('.ant-tabs-nav div').eq(2).click();

    cy.get('#idAddEtiqueta').click();

    cy.get('#clave')
        .type("pruebaClave").should('have.value', 'pruebaClave');
    cy.get('#valor')
        .type("pruebaValor").should('have.value', 'pruebaValor');
    cy.get('#descripcion')
        .type("pruebaDescrip").should('have.value', 'pruebaDescrip');

    cy.get('.acceptInAlertConfirm').click();

    cy.get('#buscadorEtiquetas')
        .type("pruebaClave").should('have.value', 'pruebaClave');

    cy.get('.bodyTagsTable tr td').eq(0).should('have.text', 'pruebaClave')
    
    cy.wait(2500)

    cy.get('.bodyTagsTable ').trigger('mouseover');
    cy.get('.bodyTagsTable ').trigger("hover");
    cy.get('.bodyTagsTable ').trigger("mouseover");
