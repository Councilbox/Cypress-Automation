...
describe("Councilbox login - valid username and password", function() {
    ...
    it("Quorum numbers (current/initial) scenario (test case 1) - current quorum 80/initial quorum 70 - 100 votes - 100 shares", function() {
    const name_census = 'Qourum'+Cypress.config('UniqueNumber')
    const meeting_title = "Meeting"+Cypress.config('UniqueNumber')
    ...
    cy.log("Click to the “Participant E”")
    ...
    cy.log("Click on the “Participant B” and observe the total votes”")
        ...
        meetingPreview.verify_owned(shares_10)
        meetingPreview.alert_close()
    ...
    cy.log("Click on the “Participant D” button")
        cy.contains(/^Participant D$/).click()
    cy.log("Click on the “Vote early” button and select the option you want")
        meetingPreview.click_on_vote_early()
        meetingPreview.alert_close_second()
        meetingPreview.alert_close()
    cy.log("Observe the current quorum number on the top right side of the page”")
        cy.wait(5000)
        meetingPreview.verify_current_quorum(quorum_2) 
    ...
    cy.log("Click on the “Delegate vote” button then choose the “Participant A” and click on it”")
        ...
        meetingPreview.alert_close()
    cy.log("Click on the “Participant A” and observe the total votes”")
        cy.contains(/^Participant A$/).click() 
        meetingPreview.verify_owned_votes(votes_20)
        meetingPreview.verify_owned(shares_10)  
        meetingPreview.alert_close()
